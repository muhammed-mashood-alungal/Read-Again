...
export const setWishlistItemsCount=(count)=>{
  return {
    type:SET_WISHLIST_ITEMS_COUNT,
    payload:count || 0
  }
}
export const incWishlistItemsCount =(incCount)=>{
  return {
    type: WISHLIST_ITEMS_COUNT_INC,
    payload:incCount
  }
}
export const decWishlistItemsCount =(decCount)=>{
  return {
    type: WISHLIST_ITEMS_COUNT_DEC,
    payload:Math.abs(decCount)
  }
}
  ...
  try{
    ...
    const response = await axiosWishlistInstance.post(`/${userId}`,{itemId})
     if(response.status == 200){
       ...
       dispatch(incWishlistItemsCount())
